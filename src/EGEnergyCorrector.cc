...
std::pair<double,double> EGEnergyCorrector::CorrectedEnergyWithErrorV3(const Photon &p, const reco::VertexCollection& vtxcol, double rho, EcalClusterLazyTools &clustertools, const edm::EventSetup &es, bool applyRescale) {
  ...
  //originally in the cmssw code
  // fVals[3]  = p.r9();
  // fVals[4]  = p.e5x5()/s->rawEnergy();
  //Below : use cluster tools calculation to be consistent with Vecbos
  fVals[3]  = clustertools.e3x3(*b) / s->rawEnergy();
  fVals[4]  = clustertools.e5x5(*b) / s->rawEnergy();
  ...
  //apply cuts to vertices
  int NGoodVertices = 0;
  for(unsigned int i = 0; i < vtxcol.size(); ++i) {
    if(vtxcol.at(i).isValid() && !vtxcol.at(i).isFake()
       && vtxcol.at(i).ndof() > 4 && fabs(vtxcol.at(i).z()) <= 24 && vtxcol.at(i).position().Rho() <= 2
       ) {
      NGoodVertices++;
    }
  }  
  fVals[10] = NGoodVertices;
  ...
  fVals[11] = (float)b->eta()-(float)s->eta();
  fVals[12] = reco::deltaPhi((float)b->phi(),(float)s->phi());
...
//   else for (int i=0; i<31; ++i) printf("%i: %5f\n",i,fVals[i]);
  ...
  // std::cout << "*********************************************************\n";
  // std::cout << "Photon : " << p.energy() << " " << p.superCluster()->eta() << " " << p.phi() << " \n";
  // for (int i=0; i < 38; ++i) {
  //   std::cout << "var: " << i << " = " << fVals[i] << "\n";
  // }
  // std::cout << "den = " << den << "\n";
  // std::cout << "Corrected Energy = " << ecor << "\n";
  // std::cout << "Corrected Energy Uncertainty = " << ecorerr << "\n";
  // std::cout << "*********************************************************\n";
